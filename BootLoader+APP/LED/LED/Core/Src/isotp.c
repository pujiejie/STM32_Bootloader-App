...
void ISOTP_Receive_Handler(uint8_t *can_data)
{
    ...
    switch (frame_type)
    {
        ...
        case 0x01: 
        {
            uint16_t pkt_len = ((can_data[0] & 0x0F) << 8) | can_data[1];
            
            // 长包: 需要连续帧
            if (pkt_len > 7 && pkt_len <= ISOTP_RX_BUFFER_SIZE)
            {
                isotp_total_len = pkt_len;
                for (uint8_t i = 0; i < 6; i++) {
            ...
            // 短包: 数据全在首帧里 (≤6 字节), 直接处理
            else if (pkt_len >= 1 && pkt_len <= 6)
            {
                for (uint8_t i = 0; i < pkt_len; i++) {
                    isotp_rx_buffer[i] = can_data[2 + i];
                }
                Process_UDS_Request(isotp_rx_buffer, pkt_len);
            }
            else
            {
                isotp_total_len = 0;
